...
int AHB2PCI_Memory_Write(u32 address, u8 size, u32 data)
{
	...
	while(!(readl(AHB_ISTATUS)&0x1))
	{
		...
		if(cnt > 1000)
		{
			printf("AHB2PCI_Memory_Write: time out! AHB_ISTATUS = 0x%04x\n", readl((AHB2PCI_BA+0x44)));
			return(false);
		}
	}

	if (AHB2PCI_IsLogEnable_g == 1)
		printf("AHB2PCI_Memory_Write: Addr=0x%X, Size=%d, Data=0x%X\n", address, size, data);
...
u32 AHB2PCI_Memory_Read(u32 address, u8 size)
{
	...
	while(!((readl(AHB_ISTATUS)&0x100)>>8))
	{
		...
		if(cnt > 1000)
		{
			printf("AHB2PCI_Memory_Read: Read transaction time out! AHB_ISTATUS = 0x%04x\n", readl(AHB_ISTATUS));
			return(0xDEADBEEF);
		}
	}
	...
	if (AHB2PCI_IsLogEnable_g == 1)
		printf("AHB2PCI_Memory_Read: Addr=0x%X, Size=%d, Data=0x%X\n", address, size, readData);
...
int AHB2PCI_Config_Write(u32 address, u8 size, u32 data, u8 iByte)
{
	...
	if (offset > 3)
	{
		printf("Config_Write: Illegal offset(iByte): %d. Must be in range 0-3\n", offset);
		return(false);
	}
	// Verify single data phase
	if((offset + size) > 4)
	{
		printf("Config_Write: Offset(%d) + size exceeding 4 byte size.\n", offset);
		return(false);
	}
	...
	writel((data & GENMASK(size * 8, 0)) << (offset * 8), DATA_REG);
	...
	while(!(readl(AHB_ISTATUS)&0x1))
	{
		...
		if(cnt > 1000)
		{
			printf("AHB2PCI_Config_Write: time out! AHB_ISTATUS = 0x%04x\n", readl(AHB_ISTATUS));
			return(false);
		}
	}

	if (AHB2PCI_IsLogEnable_g == 1)
		printf("AHB2PCI_Config_Write: wrote to address=0x%x, size=%d, data=0x%x, ibyte=%d\n", address, size, data, offset);
...
u32 AHB2PCI_Config_Read(u32 address, u8 size)
{
	...
	while(!((readl(AHB_ISTATUS)&0x100)>>8))
	{
		...
		if(cnt > 1000)
		{
			printf("AHB2PCI_Config_Read: Read transaction time out! AHB_ISTATUS = 0x%04x\n", readl(AHB_ISTATUS));
			return(0xDEADBEEF);
		}
	}
	...
	aligned_data = (readData >> (offset * 8)) & GENMASK(size * 8, 0);

	if (AHB2PCI_IsLogEnable_g == 1)
		printf("AHB2PCI_Config_Read: read from address=0x%x, size=%d, data=0x%x (DW data=0x%x)\n", address, size, aligned_data, readData);
...
{
	FB_BA = getFrameBufferAddress();

	printf("FB_Initialize(): Frame-Buffer address = %08xh\n", FB_BA);
	return 1;
...
int PLL_Configure (u8 _GPLLINDIV, u8 _GPLLFBDIV, u8 _GPLLST)
{
	...
	printf("  * _GPLLINDIV (INDV + FBDV8): %08xh\n", AHB2PCI_Memory_Read((MGABASE1 + 0x3C08), sizeof(u32)));
	...
	printf("  * _GPLLST (OTDV1/OTDV2): %08xh\n", AHB2PCI_Memory_Read((MGABASE1 + 0x3C08), sizeof(u32)));
...
void GFX_ClearMemorySpace(u8 presetValue)
{
	uintptr_t ptr = FB_BA;
	if(FB_Initialize() == 1)
		memset((char *)ptr, presetValue, (16 * 1024 * 1024));
	else
...
int GFX_ConfigureDisplayTo1920x1200(GFX_ColorDepth colorDepth)
{
	...
	if (MGABASE1 == 0)
	{
		printf("TEST_ConfigureDisplay failed : MGABASE1 is invalid = %08xh\n", MGABASE1);
	}
	else
	{
		printf("TEST_ConfigureDisplay : MGABASE1 = %08xh\n", MGABASE1);
	}
	AHB2PCI_Config_Write(GFX_IDSEL + GFX_BAR2, sizeof(u32), 0xf0000000, 0);

	printf("MGABASE1 = %08xh, MGABASE2 = %08xh\n",
...
int GFX_ConfigureDisplayTo1024x768(GFX_ColorDepth colorDepth)
{
	...
	if (!AHB2PCI_Memory_Write(CRTC, sizeof(u16), 0x2606)){return (false);} // TOTAL		// was 24
...
int GFX_Draw_Pattern(int out_color)
{
	...
	u32 hRes = 0, vRes = 0, colorDepth = 0 , Index, Color, lineNum=0;
	int cnt=0;
	const u32 numOfBars=16;
	uintptr_t Address;
...
void dumpGFX_InfoRegs (u32 *hRes, u32 *vRes, u32 *colorDepth, u32 *pll_freq)
{
	...
	*hRes = 0; *vRes = 0; *colorDepth = 0; *pll_freq = 0;
	...
	printf("> Calculated PLLG output: %u[MHz] (rounded)\n", *pll_freq);
...
static int do_gfx_command(struct cmd_tbl *cmdtp, int flag, int argc, char * const argv[])
{
	int	command;
	int rc = 0, ret = 1;
	...
	case 2:
			...
			if(!strcmp(argv[2], "on"))
			{
				...
				clrbits_le32(PCIRREL_REG, PCIRREL_BIT);
			...
			else if(!strcmp(argv[2], "off"))
			{
				...
				setbits_le32(PCIRREL_REG, PCIRREL_BIT);
	...
	case 13:  //phy power down
			...
			printf(">Phy power down INTCR2= 0x%08x \n", readl(INTCR2));
			break;
